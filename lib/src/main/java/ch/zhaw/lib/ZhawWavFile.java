...
public class ZhawWavFile implements AutoCloseable
{
	...
	public static ZhawWavFile newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, ZhawWavFileException
	{
		// Instantiate new Wavfile and initialize
		ZhawWavFile wavFile = new ZhawWavFile();
		wavFile.file = file;
		...
		if (numChannels < 1 || numChannels > 65535) throw new ZhawWavFileException("Illegal number of channels, valid range 1 to 65536");
		if (numFrames < 0) throw new ZhawWavFileException("Number of frames must be positive");
		if (validBits < 2 || validBits > 65535) throw new ZhawWavFileException("Illegal number of valid bits, valid range 2 to 65536");
		if (sampleRate < 0) throw new ZhawWavFileException("Sample rate must be positive");
	...
	public static ZhawWavFile openWavFile(File file) throws IOException, ZhawWavFileException
	{
		...
		if (bytesRead != 12) throw new ZhawWavFileException("Not enough wav file bytes for header");
		...
		if (riffChunkID != RIFF_CHUNK_ID) throw new ZhawWavFileException("Invalid Wav Header data, incorrect riff chunk ID");
		if (riffTypeID != RIFF_TYPE_ID) throw new ZhawWavFileException("Invalid Wav Header data, incorrect riff type ID");

		// Check that the file size matches the number of bytes listed in header
		if (file.length() != chunkSize+8) {
			throw new ZhawWavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + file.length() + ")");
		}
		...
		while (true)
		{
			...
			if (bytesRead == -1) throw new ZhawWavFileException("Reached end of file without finding format chunk");
			if (bytesRead != 8) throw new ZhawWavFileException("Could not read chunk header");
			...
			if (chunkID == FMT_CHUNK_ID)
			{
				...
				if (compressionCode != 1) throw new ZhawWavFileException("Compression Code " + compressionCode + " not supported");
				...
				if (wavFile.numChannels == 0) throw new ZhawWavFileException("Number of channels specified in header is equal to zero");
				if (wavFile.blockAlign == 0) throw new ZhawWavFileException("Block Align specified in header is equal to zero");
				if (wavFile.validBits < 2) throw new ZhawWavFileException("Valid Bits specified in header is less than 2");
				if (wavFile.validBits > 64) throw new ZhawWavFileException("Valid Bits specified in header is greater than 64, this is greater than a long can hold");

				// Calculate the number of bytes required to hold 1 sample
				wavFile.bytesPerSample = (wavFile.validBits + 7) / 8;
				if (wavFile.bytesPerSample * wavFile.numChannels != wavFile.blockAlign)
					throw new ZhawWavFileException("Block Align does not agree with bytes required for validBits and number of channels");
			...
			else if (chunkID == DATA_CHUNK_ID)
			{
				...
				if (foundFormat == false) throw new ZhawWavFileException("Data chunk found before Format chunk");

				// Check that the chunkSize (wav data length) is a multiple of the
				// block align (bytes per frame)
				if (chunkSize % wavFile.blockAlign != 0) throw new ZhawWavFileException("Data Chunk size is not multiple of Block Align");
		...
		if (foundData == false) throw new ZhawWavFileException("Did not find a data chunk");
	...
	private long readSample() throws IOException, ZhawWavFileException
	{
		long val = 0;
		...
		for (int b=0 ; b<bytesPerSample ; b++)
		{
			if (bufferPointer == bytesRead) 
			{
				...
				if (read == -1) throw new ZhawWavFileException("Not enough data available");
				bytesRead = read;
				bufferPointer = 0;
			}
	...
	// Integer
	// -------
	public int readFrames(int[] sampleBuffer, int numFramesToRead) throws IOException, ZhawWavFileException
	{
		return readFrames(sampleBuffer, 0, numFramesToRead);
	}

	public int readFrames(int[] sampleBuffer, int offset, int numFramesToRead) throws IOException, ZhawWavFileException
	{
	...
	public int writeFrames(int[] sampleBuffer, int numFramesToWrite) throws IOException, ZhawWavFileException
	{
		return writeFrames(sampleBuffer, 0, numFramesToWrite);
	}

	public int writeFrames(int[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, ZhawWavFileException
	{
	...
	// Long
	// ----
	public int readFrames(long[] sampleBuffer, int numFramesToRead) throws IOException, ZhawWavFileException
	{
		return readFrames(sampleBuffer, 0, numFramesToRead);
	}

	public int readFrames(long[] sampleBuffer, int offset, int numFramesToRead) throws IOException, ZhawWavFileException
	{
		if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");
	...
	public int writeFrames(long[] sampleBuffer, int numFramesToWrite) throws IOException, ZhawWavFileException
	{
		return writeFrames(sampleBuffer, 0, numFramesToWrite);
	}

	public int writeFrames(long[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, ZhawWavFileException
	{
		if (ioState != IOState.WRITING) throw new IOException("Cannot write to WavFile instance");
	...
	// Double
	// ------
	public int readFrames(double[] sampleBuffer, int numFramesToRead) throws IOException, ZhawWavFileException
	{
		return readFrames(sampleBuffer, 0, numFramesToRead);
	}

	public int readFrames(double[] sampleBuffer, int offset, int numFramesToRead) throws IOException, ZhawWavFileException
	{
		if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");
	...
	public int writeFrames(double[] sampleBuffer, int numFramesToWrite) throws IOException, ZhawWavFileException
	{
		return writeFrames(sampleBuffer, 0, numFramesToWrite);
	}

	public int writeFrames(double[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, ZhawWavFileException
	{
		if (ioState != IOState.WRITING) throw new IOException("Cannot write to WavFile instance");
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			{
				ZhawWavFile readWavFile = openWavFile(new File(filename));
				readWavFile.display();
				...
				ZhawWavFile writeWavFile = newWavFile(new File("out.wav"), numChannels, numFrames, validBits, sampleRate);
...
//				long[] buffer = new long[BUF_SIZE * numChannels];
			...
			ZhawWavFile writeWavFile = newWavFile(new File("out2.wav"), 1, 10, 23, 44100);
